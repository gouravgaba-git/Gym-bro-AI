  ...
  return (
    <header className="sticky top-0 z-50 backdrop-blur-xl bg-[#070a14]/85 border-b border-white/10 shadow-2xl transition-all duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 sm:h-20 flex items-center justify-between relative">
        ...
        <div
          className="flex items-center gap-2.5 sm:gap-3 cursor-pointer group absolute left-1/2 -translate-x-1/2 md:static md:translate-x-0"
          onClick={() => {
        ...
        <nav className="hidden md:flex items-center gap-1.5 p-1.5 rounded-full bg-[#0d1322]/90 border border-white/10 shadow-inner">
          ...
          <button
            onClick={() => handleProtectedNav("/profile", "Please sign in with Google to access your personal profile.")}
          ...
          <button
            onClick={() => handleProtectedNav("/settings", "Please sign in with Google to access your account settings.")}
            className="px-5 py-2 rounded-full text-xs font-bold transition-all duration-300 flex items-center gap-2 text-gray-400 hover:text-white hover:bg-white/5 cursor-pointer"
          >
            <span>⚙️</span>
            <span>Settings</span>
        ...
        <div className="flex items-center gap-3" ref={dropdownRef}>
          {!isAuthenticated ? (
            <button
              ...
              onClick={() => openAuthModal("Sign in with Google to save custom workout splits.")}
            >
            ...
            <div className="relative">
              ...
              {/* User Dropdown Menu */}
              {dropdownOpen && (
                <div className="absolute right-0 mt-3 w-64 bg-[#0d1322]/95 border border-white/10 rounded-3xl shadow-2xl p-3 backdrop-blur-2xl z-50 space-y-1">
      ...
      {/* Mobile Nav Drawer */}
      {mobileMenuOpen && (
        <div className="md:hidden border-t border-white/10 bg-[#070a14] px-4 py-4 space-y-2">
          <NavLink
            to="/dashboard"
          ...
          <button
            onClick={() => {
              setMobileMenuOpen(false);
              handleProtectedNav("/profile", "Please sign in with Google to access profile.");
            }}
          ...
          <button
            onClick={() => {
              setMobileMenuOpen(false);
              handleProtectedNav("/settings", "Please sign in with Google to access settings.");
            }}
            className="w-full text-left px-4 py-3 rounded-2xl text-xs font-extrabold flex items-center gap-3 text-gray-300 bg-[#0d1322] hover:bg-white/10 cursor-pointer border border-white/5"
          >
            <span>⚙️</span>
